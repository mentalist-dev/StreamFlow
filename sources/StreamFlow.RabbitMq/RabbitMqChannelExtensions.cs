...
public static class RabbitMqChannelExtensions
{
        ...
        , Dictionary<string, object>? additionalArguments = null)
    {
        ...
        if (queueOptions?.Arguments.Count > 0)
        {
            arguments ??= new Dictionary<string, object>();
    ...
    public static void TryCreateExchange(this IModel channel
        , ILogger logger
        , string exchange
        , bool? autoDelete = null
        , Dictionary<string, object>? additionalArguments = null)
    {
        ...
        var exchangeAutoDelete = autoDelete ?? false;

        try
        {
            logger.LogInformation(
                "Declaring exchange: [{ExchangeName}].",
                exchange);

            channel.ExchangeDeclare(exchange, exchangeType, true, exchangeAutoDelete, arguments);
        }
        catch (OperationInterruptedException e)
        {
            // 406: PRECONDITION_FAILED, queue exists, but with different properties
            if (e.ShutdownReason?.ReplyCode != Constants.PreconditionFailed)
            {
                logger.LogError(e, "Unable to declare exchange [{ExchangeName}].", exchange);
                throw;
            }

            logger.LogWarning(e
                , "Exchange already exists with name [{ExchangeName}] but different options. Expected exchange type = '{ExchangeType}' and Auto Delete = '{ExchangeAutoDelete}'. {ExchangeArguments}"
                , exchange, exchangeType, exchangeAutoDelete, arguments);
        }
        catch (Exception e)
        {
            logger.LogError(e, "Unable to declare exchange [{ExchangeName}].", exchange);
